 ...
 * @param {string} [slug] - The filter value (category slug, author ID, or tag ID).
 ...
 * @since 1.1.0
 */
export default async function Article({ data, type, page = 1, perPage = 10 }: {
	data: string, type?: string, page?: number, perPage?: number
}): Promise<JSX.Element> {

	// Add pagination params
	const api = `&per_page=${ perPage }&page=${ page }`
	...
	return (
		<main className="flex min-h-screen flex-col justify-between p-24">
			...
			<div className="flex justify-center mt-8 space-x-4">
				{ page > 1 && (
					<Link
						href={ `?type=${ type ?? '' }&slug=&page=${ page - 1 }&per_page=${ perPage }` }
